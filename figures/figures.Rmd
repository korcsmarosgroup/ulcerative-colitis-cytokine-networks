...
# nichenet interactions from omnipath
networks <- readRDS("OP_nichenet_networks.RDS")
...
results <- list()
for (row in 1:nrow(testcci)){
src <- testcci[row, 'source']
trg <- testcci[row, 'target']
...
st1<- ('STAT1' %in% el$from | 'STAT1' %in% el$to) |> as.logical() |> as.integer()
st2<- ('STAT2' %in% el$from | 'STAT2' %in% el$to) |> as.logical() |> as.integer()
st3<- ('STAT3' %in% el$from | 'STAT3' %in% el$to) |> as.logical() |> as.integer()
st4<- ('STAT4' %in% el$from | 'STAT4' %in% el$to) |> as.logical() |> as.integer()
st5a<- ('STAT5a' %in% el$from | 'STAT5a' %in% el$to) |> as.logical() |> as.integer()
st5b<- ('STAT5b' %in% el$from | 'STAT5b' %in% el$to) |> as.logical() |> as.integer()
st6<- ('STAT6' %in% el$from | 'STAT6' %in% el$to) |> as.logical() |> as.integer()

outdf <- data.frame(src,trg,jk1,jk2,jk3,tk2,st1,st2,st3,st4,st5a,st5b,st6) |> tibble()
results[[paste0(src, '_',trg)]] <- outdf
}

jaknet<-bind_rows(results)
jaknet$jaks <- rowSums( jaknet[,3:6] )

write_tsv(jaknet, 'cci_jak10_stat.tsv')
```

```{r}
#Fig5A
jaki <- read_tsv('cci_jak10_stat.tsv') |> dplyr::rename(JAK1 = jk1, JAK2 = jk2, JAK3 = jk3, TYK2 = tk2)
jak_usage<-jaki |> select(source, target,jaks) |> group_by(jaks) |> tally()
...
jakil<-jaki |> pivot_longer(cols = JAK1:TYK2, names_to = 'jak', values_to = 'presence')
...
jak_protein_usage_plot<-ggplot(jak_protein_usage, aes(x=jaks, y=n, fill=jak)) + 
  ...
  theme_hc(base_size = 16)+
  scale_fill_manual(values = c('JAK1'='#e1f7d5', 'JAK2'='#f6a6b2', 'JAK3'='#f7c297', 'TYK2'='#eecbff'))+  coord_flip()+
  coord_flip()+
  scale_x_reverse()+
  ylab('Ratio of JAK proteins')+
  theme(plot.margin = margin(1,1,1.5,1.2, "cm"))+
  ylab('Number of JAK proteins')+ guides(fill=guide_legend(title="JAK"))
