...
```{r}
# using the Cui et al supplementary data to validate cytokine networks
cklcyts <- c(comp$source, comp$target) |> unique()
cyts <- read_tsv('../cytokine_list.tsv', col_names = F) |> pull(X1) |> unique()
...
```{r}
#Fig5C
#rewiring values calculated in cytoscape following the removal of interactions predicted to be signalling through each JAK paralogue
...
```{r}
library(ggpubr)
# OT data downloaded from website
...
#top onteragets score cytokines
